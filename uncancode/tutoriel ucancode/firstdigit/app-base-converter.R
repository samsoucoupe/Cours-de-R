...
DecodeNumber <- function(word, base) {
  #convertir la chaîne de caractère en une liste de chiffres
  digits <- DecodeDigits(word)
  #vérifier que les chiffres sont dans la base
  if (any(digits >= base)) {
    return("NA")
  }
  #convertir la liste de chiffres en un nombre en base base
  n <- 0
  for (i in 1:length(digits)) {
    n <- n + digits[i] * base ^ (length(digits) - i)
  }
  return(n)
}
stopifnot( DecodeNumber("121", 10)=="121")
stopifnot( DecodeNumber("1010", 2)=="10")
stopifnot( DecodeNumber("1A", 16)=="26")
stopifnot( DecodeNumber("1Az", 36)=="1691")
stopifnot(DecodeNumber("121A", 10) == "NA")
...
EncodeNumber <- function(n, base) {
  #convertir le nombre n en une liste de chiffres
  digits <- c()
  while (n > 0) {
    ...
    #convertir la liste de chiffres en une chaîne de caractères
    alphabet <- c(0:9, LETTERS)
    return(paste(alphabet[digits + 1], collapse = ""))

}

stopifnot(EncodeNumber(60, 10)=="60")
stopifnot(EncodeNumber(60, 2)=="111100")
stopifnot(EncodeNumber(60, 16)=="3C")
stopifnot(EncodeNumber(61, 36)=="1P")
...
EncodeBibi <- function(n) {
  #alphabet bibi-binaire

  #convertir le nombre n en une liste de chiffres
  digits <- c()
  while (n > 0) {
    digits <- c( n %% 16,digits)
    n <- n %/% 16
  }

   #convertir la liste de chiffres en une chaîne de caractères
   alphabet <- c("HO","HA","HE","HI","BO","BA","BE","BI","KO","KA","KE","KI","DO","DA","DE","DI")
   return(paste(alphabet[digits + 1], collapse = ""))
}

stopifnot(EncodeBibi(60)=="HIDO")
stopifnot(EncodeBibi(2018)=="BIDEHE")
