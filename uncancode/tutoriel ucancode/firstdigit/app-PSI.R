...
SummaryPSI <- function(x) {
  #calculer les indicateurs statistiques
    ...
    return(c(n = n, mean = mean, median = median, sd = sd, min = min, max = max))
...
HistPSI <- function(x, main, binwidth=0.5) {
  ...
  if(require("ggplot2")) {
    ...
    ggplot(data.frame(x=x), aes(x=x)) + geom_histogram(binwidth=binwidth) + labs(title=main)
...
hist(SampleMeans(1:10,100,5))
SampleMeans(c(1,2,3,4,5,8,9,10),2,5)
